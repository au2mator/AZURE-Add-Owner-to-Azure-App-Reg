...
# Code Template V 1.5
#
# Github: https://github.com/au2mator/AZURE-Add-Owner-to-Azure-App-Reg
#
#
...
param (
    ...
    [String]$c_ApplicationRegistration,
...
[string]$LogfileName = "AZURE - Add Owner to Azure App Reg"
[string]$CredentialStorePath = "C:\_SCOworkingDir\TFS\PS-Services\CredentialStore" #see for details: https://click.au2mator.com/PSCreds/?utm_source=github&utm_medium=social&utm_campaign=Azure_AddOwnerAzureAppReg&utm_content=PS1
...
$ChannelName = "General"
###TeamsCredentials to send Teams Card #https://click.au2mator.com/PSCreds/?utm_source=github&utm_medium=social&utm_campaign=Azure_AddOwnerAzureAppReg&utm_content=PS1
...
###Stored Credentials
###See: https://click.au2mator.com/PSCreds/?utm_source=github&utm_medium=social&utm_campaign=Azure_AddOwnerAzureAppReg&utm_content=PS1
...
try {
    ...
    try {
        ...
        try {
            Write-au2matorLog -Type INFO -Text "TRY to Get App ID from Name"

            $GetIDfromName_Params = @{
                Method = "GET"
                Uri    = "$MSGRAPHAPI_BaseURL/applications?`$filter=displayName eq '$c_ApplicationRegistration'"
                header = $MSGRAPHAPI_headers
            }        
        
            $GetIDfromName_Result = Invoke-RestMethod @GetIDfromName_Params
            Write-au2matorLog -Type INFO -Text "Received this ID for the App Reg: $($GetIDfromName_Result.value.id)"



            Write-au2matorLog -Type INFO -Text "TRY to add the Owner to Azure App Reg"
...
"@
            ...
            $SetRegAppOwner_Params = @{
                Method = "POST"
                Uri    = "$MSGRAPHAPI_BaseURL/applications/$($GetIDfromName_Result.value.id)/owners/`$ref"
                header = $MSGRAPHAPI_headers
            ...
            Write-au2matorLog -Type INFO -Text "Owner is added to the Azure App Reg"

            $au2matorReturn = "Owner is added to the Azure App Reg"
            $TeamsReturn = "Owner is added to the Azure App Reg" #No Special Characters allowed
            $AdditionalHTML = "Owner is added to the Azure App Reg
            <br>
                "
            $Status = "COMPLETED"
        }
            ...
            Write-au2matorLog -Type ERROR -Text "Failed to add a new Owner"
            Write-au2matorLog -Type ERROR -Text $Error
        
            $au2matorReturn = "Failed to add a new Owner, Error: $Error"
            $TeamsReturn = "Failed to add a new Owner" #No Special Characters allowed
            $AdditionalHTML = "Failed to add a new Owner
            <br>
            Error: $Error
                "
            $Status = "ERROR"
